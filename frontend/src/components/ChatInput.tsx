...
interface ChatInputProps {
  // Recibe la mutación del padre para unificar estado (evita duplicar mutaciones)
...
export function ChatInput({ sendMutation }: ChatInputProps) {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!trimmed) return  // Validación cliente: no vacío ni solo espacios

    // Se limpia al enviar; si falla se restaura para no perder lo escrito
    setContenido('')
    sendMutation.mutate(
      { contenido: trimmed },  // Body: { contenido } - SIN direccion (server fija 'saliente')
      { 
        // Rollback visual si falla: restaura el texto en el textarea
  ...
  // ============================================
  // KEYBOARD HANDLER: Enter = enviar, Shift+Enter = nueva línea
  // ============================================
    ...
    <form onSubmit={handleSubmit} className="w-full shrink-0">
      <div className="p-4 safe-bottom border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
        <div className="flex gap-2 items-end">
          ...
          <button
            type="submit"
            disabled={!contenido.trim() || sendMutation.isPending}  // Deshabilitado si vacío o enviando
        ...
        {/* ============================================
            PISTA DE TECLADO (fuera del placeholder para no desbordar en móvil)
            ============================================ */}
        ...
        {/* ============================================
            ERROR DE ENVÍO (role="alert" para screen readers)
            ============================================ */}
